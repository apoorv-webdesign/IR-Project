...
def create_unigram_index(filename):
	...
	for k,v in c.iteritems():
		...
		if k not in dict_unigram_index:
			...
			dict_unigram_index[k].append((filename.split('\\', 1)[-1].split('.')[0],v))
		else:
			dict_unigram_index[k].append((filename.split('\\', 1)[-1].split('.')[0],v))
	f.close()
...
def unigram_index():
	for filename in glob.glob('Cacm_tok\*.txt'):
		create_unigram_index(filename)
	with open("unigram_index.txt",'w') as input_file:
		for i,j in dict_unigram_index.iteritems():
			line=i[0]+' -> '+str(j).strip('[').strip(']')+'\n'
